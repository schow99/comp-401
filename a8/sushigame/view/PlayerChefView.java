...
public class PlayerChefView extends JPanel implements ActionListener {
	...
	public void refreshPanel() {
		...
		if (currentSush == 2) {
			...
			toppings = new JComboBox(comp401sushi.Sashimi.SashimiType.values());
			add(toppings);
			...
			toppings = new JComboBox(comp401sushi.Nigiri.NigiriType.values());
			...
			for (int i = 0; i < ingredients.length; i++) {
				if (ingredients[i].isSelected() && !ounces[i].getText().equals("")) {
					...
					if (amounts > 1.5) {
						...
						if (i == 0) {
							yummy_stuff.add(new YellowtailPortion(amounts));
						} else if (i == 1) {
